...
class MenuDeliveryFragment : Fragment() {
    ...
    private fun submitToFirebase(it: DeliveryModel?) {
        ...
        delivpRef.addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onDataChange(p0: DataSnapshot) {
                ...
                data2.child("invoiceNumber").setValue(it!!.invoiceNumber)
    ...
    private fun showDialogStatus(root: View) {
        ...
        val txtShipper_create = itemView.findViewById<EditText>(R.id.txt_shipper_delivery_create_task)
        val txtInvoice_create = itemView.findViewById<EditText>(R.id.txt_invoice_delivery_create_task)
        val txtEndClient_create = itemView.findViewById<EditText>(R.id.txt_end_client_delivery_create_task)
...
//            }
            if (!TextUtils.isEmpty(txtShipper_create.text.toString()) && !TextUtils.isEmpty(txtInvoice_create.text.toString())
                && !TextUtils.isEmpty(txtEndClient_create.text.toString()) && !TextUtils.isEmpty(txtAddress_create.text.toString()) &&
                !TextUtils.isEmpty(txtPhone_create.text.toString()) && !TextUtils.isEmpty(txtDescriptio_create.text.toString())){
                ...
                statusModel!!.shipper = txtShipper_create!!.text.toString().toUpperCase()
                statusModel!!.phone = txtPhone_create!!.text.toString()
                statusModel!!.invoiceNumber = txtInvoice_create!!.text.toString()
                statusModel!!.endClientName = txtEndClient_create!!.text.toString()
